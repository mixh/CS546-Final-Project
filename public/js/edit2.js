...
} else if (userGender === "prefer not to say") {
...
window.addEventListener("load", function () {
  // Second function
  const x = document.getElementById("place");
  ...
  // Preselect an option based on the value of {{user.place}}
  const userPlace = decodeURIComponent(url.searchParams.get("bucketlist").replace(/\+/g, " "));
  if (userPlace) {
    x.value = userPlace;
  }
});
...
window.addEventListener("load", function () {
  // First function
  const y = document.getElementById("gym");

  fitnessOptions.forEach((a) => {
    const option = document.createElement("option");
    option.text = a;
    option.value = a;
    y.add(option);
  ...
  const userGym = decodeURIComponent(url.searchParams.get("gym").replace(/\+/g, " "));
  if (userGym) {
    y.value = userGym;
  }
});
